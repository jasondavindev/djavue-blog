...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=djavue-blog.example.com
...
workon djavue-blog  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development djavue-blog npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop djavue-blog
    docker rm djavue-blog
    docker run --name djavue-blog -d --env-file /home/ubuntu/djavue-blog.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/djavue-blog:/dkdata \
        djavue-blog start_web.sh
}
...
echo_green "Welcome to djavue-blog's dev env"
